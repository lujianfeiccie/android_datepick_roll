...
/**
 * 用法
 * wv_province = (WheelView) findViewById(R.id.wv_ymymapactivity_province);
 wv_city = (WheelView) findViewById(R.id.wv_ymymapactivity_city);

 wv_province.setVisibleItems(5);
 ArrayWheelAdapter<String> adapter =
 new ArrayWheelAdapter<String>(this, province);

 adapter.setTextSize(14);
 wv_province.setViewAdapter(adapter);

 wv_city.setVisibleItems(5);
 ArrayWheelAdapter<String> cityAdapter =
 new ArrayWheelAdapter<String>(this, city[0]);
 cityAdapter.setTextSize(14);
 wv_city.setViewAdapter(cityAdapter);
 wv_city.setCurrentItem(city[0].length / 2);
 ...
 ArrayWheelAdapter<String> adapter =
 new ArrayWheelAdapter<String>(MyAreaChooseActivity.this, city[newValue]);
 adapter.setTextSize(14);
 wv_city.setViewAdapter(adapter);
 wv_city.setCurrentItem(city[newValue].length / 2);
 }
 });
 wv_city.setCurrentItem(2);

 bt_confirm = (Button) findViewById(R.id.bt_ymymapactivity_confirm);
 ...
 bt_cancel = (Button) findViewById(R.id.bt_ymymapactivity_cancel);
 ...
 provinceList = helper.areaNode();
 province = new String[provinceList.size()];
 city = new String[provinceList.size()][];
 for (int i = 0; i < provinceList.size(); i++) {
 province[i] = provinceList.get(i);
 cityList = helper.areaPairNode(i + 1);
 city[i] = new String[cityList.size()];
 for (int j = 0; j < cityList.size(); j++) {
 city[i][j] = cityList.get(j);
 }
 }
...
public class WheelView extends View {
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (!isEnabled() || getViewAdapter() == null) { return true; }
	...
	private ItemsRange getItemsRange() {
		if (getItemHeight() == 0) { return null; }
	...
	private View getItemView(int index) {
		if (viewAdapter == null || viewAdapter.getItemsCount() == 0) { return null; }
